...
case class ArrayMulDataModule(len: BigInt) {
  ...
  def trans(
      ...
      regs: ArrayMulDataModuleRegs
  ): (ArrayMulDataModuleOutputs, ArrayMulDataModuleRegs) = {
    ...
    Range(0, len, 2).foreach((i: BigInt) => {
      ...
      val t = MuxLookup(
        last_x,
        Lit(0, 2).U,
        List((Lit(4).U -> Lit(2, 2).U), (Lit(5).U -> Lit(1, 2).U), (Lit(6).U -> Lit(1, 2).U))
      )
      last_x = x
      val (pp, weight) =
        if ((i == 0)) (Cat(List(~s, s, s, pp_temp)), 0)
        else if (((i == (len - 1)) || (i == (len - 1)))) (Cat(List(~s, pp_temp, t)), (i - 2))
        else (Cat(List(Lit(1, 1).U, ~s, pp_temp, t)), (i - 2))
      (0 until columns.length).foreach((j: BigInt) =>
        if (((j >= weight) && (j < (weight + pp.width)))) columns = columns.updated(j, (columns(j) :+ pp((j - weight))))
      )
    })
    ...
    def addAll(cols: List[List[Bool]], depth: BigInt): (UInt, UInt) = {
      ...
      if (cols.forall((x$7: List[Bool]) => (x$7.size <= 2))) {
        ...
        (
          Cat(cols.map((x$8: List[Bool]) => x$8(0)).reverse),
          Cat(Cat(cols.drop(k).map((x$9: List[Bool]) => x$9(1)).reverse), Lit(0, k).U)
        )
      } else {
  ...
  def arrayMulDataModuleRun(
      timeout: Int,
      inputs: ArrayMulDataModuleInputs,
      regInit: ArrayMulDataModuleRegs
  ): (ArrayMulDataModuleOutputs, ArrayMulDataModuleRegs) = {
  ...
  def run(
      inputs: ArrayMulDataModuleInputs,
      randomInitValue: ArrayMulDataModuleRegs
  ): (ArrayMulDataModuleOutputs, ArrayMulDataModuleRegs) = {
