...
case class Div(
  ...
  def trans(inputs: DivInputs, regs: DivRegs): (DivOutputs, DivRegs) = {
    ...
    val (s_ready, s_neg_inputs, s_div, s_dummy, s_neg_output, s_done_div) =
      (Lit(0, 3).U, Lit(1, 3).U, Lit(2, 3).U, Lit(3, 3).U, Lit(4, 3).U, Lit(5, 3).U)
    ...
    if ((divUnroll != 0)) if (when((regs.state === s_div))) {
      unrolls = (0 until divUnroll)
        .scanLeft(regs.remainder)({
          case (rem, i) => {
    ...
    if (when((io_req_ready && inputs.io_req_valid))) {
      ...
      resHi_next = resHi_next := Lit(false).B
      count_next =
        count_next := (if (fastMulW)
                         Mux((cmdMul && halfWidth(inputs.io_req_bits_dw)), Lit(((w / mulUnroll) / 2)).U, Lit(0).U)
                       else Lit(0).U)
